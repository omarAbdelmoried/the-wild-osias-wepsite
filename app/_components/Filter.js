...
function Filter({ options, defaultValue, paramsName }) {
  ...
  return (
    <div className="flex gap-1 items-center border-2 border-primary-800">
      {options.map((filter) => (
        <Button
          key={filter.value}
...
function Button({ handleFilter, activeFilter, filter, children }) {
  return (
    ...
    <button
      ...
      className={`px-5 py-2 hover:bg-primary-700 hover:text-primary-50 transition-colors ${activeFilter === filter ? "bg-primary-800" : ""}`}
